angular.module('starter.controllers', [])
    ...
    .controller('ClientDetailCtrl', function ($scope, $state, $stateParams, $filter, $ionicPopup, $rootScope, $ionicLoading, $cordovaToast, $cordovaDialogs, Server) {
        ...
        $scope.updateClient = function () {
            ...
            var client = {
                "token": token,
                "first_name": $scope.clientData.first_name,
                "last_name": $scope.clientData.last_name,
                "phone": $scope.clientData.phone,
                "client_email": $scope.clientData.client_email,
                "address": $scope.clientData.address,
                "city": $scope.clientData.city,
                "state": $scope.clientData.state,
                "zipcode": $scope.clientData.zipcode
            }
...
// Distributor List
    ...
    .controller('DistributorAddCtrl', function ($scope, $rootScope, $stateParams, $ionicLoading, $ionicHistory, $cordovaToast, $cordovaDialogs, Server, ImageService) {
        var loading = function () {
            $ionicLoading.show({
                template: '<ion-spinner style="fill: #fff" icon="bubbles"></ion-spinner>' +
                '<br/><p>Processing...</p>'
        ...
        $scope.editDistributor = function () {
            ...
            var disData = {
                ...
                "image": $scope.distributor.image,
                "phone": $scope.distributor.phone,
                "distributor_email": $scope.distributor.distributor_email,
                "fax": $scope.distributor.fax,
                "address": $scope.distributor.address,
                "city": $scope.distributor.city,
                "state": $scope.distributor.state,
                "zip": $scope.distributor.zip,
                "sales_name": $scope.distributor.sales_name,
                "sales_phone": $scope.distributor.sales_phone,
                "sales_email": $scope.distributor.sales_email
            }
            $ionicLoading.show();
...
//Product Details Page
    ...
    .controller('ProductDetailCtrl', function ($scope, $ionicModal, $state, $stateParams, $rootScope, $cordovaDialogs, $ionicLoading, $stateParams, $filter, Server) {
        ...
        $scope.qrCode = {
            ...
            distributor_id: $scope.product.distributor_id,
            material: $scope.product.material,
            project: '0',
            min_order_size: $scope.product.min_order_size,
            available: $scope.product.available,
            cost: $scope.product.cost, protection: $scope.product.protection,
            material_fiber: $scope.product.material_fiber,
            pattern: $scope.product.pattern,
            unit: $scope.product.unit,
            mark_up_percent: $scope.product.mark_up_percent,
            length: $scope.product.length,
            width: $scope.product.width,
            area: $scope.product.area,
            min_shipping_cost: $scope.product.min_shipping_cost,
            flat_fee: $scope.product.flat_fee,
    ...
    .controller('ProductAddCtrl', function ($scope, $stateParams, $rootScope, $ionicModal, $timeout, $ionicHistory, $cordovaDialogs, $ionicLoading, Server, ImageService, $filter) {
        ...
        if (productData.name) {
            $scope.product = {
                ...
                sku_num: productData.sku_num,
                min_order_size: productData.min_order_size,
                project: '0',
                distributor_id: productData.distributor_id,
                available: productData.available,
                protection: productData.protection,
                warr_info: productData.warr_info,
                cost: productData.cost,
                mark_up_percent: productData.mark_up_percent,
                meterial_fiber: productData.material_fiber,
                length: productData.length,
                width: productData.width,
                pattern: productData.pattern,
                area: productData.area,
                unit: productData.unit,
                min_shipping_cost: productData.min_shipping_cost,
                flat_fee: productData.flat_fee,
    ...
    .controller('OrderDetailCtrl', function ($scope, $rootScope, $state, $stateParams, $ionicModal, $filter, $ionicHistory, $ionicPopup, $ionicLoading, $cordovaToast, $cordovaDialogs, Server, ionicDatePicker, ionicTimePicker, InvoiceService, ImageService) {
        $scope.step1 = false;
        $scope.step2 = false;
        $scope.step3 = false;
        $scope.step4 = false;
            ...
            Server.httpDetails(method, url1, '').then(function (response) {
                ...
                if (response.status == 200) {
                    ...
                    sort_products = $scope.productList.filter(function (product) {
                        const item = $scope.productData.find(function (product1) {
        ...
        $scope.uploadPicture = function () {
            ImageService.getPictureOptions().then(function (file) {
                ImageService.uploadImage(file).then(function (url) {
                    console.log('3', url);
                    $scope.order.attach_image = url;
                    $scope.uploadAttachInvoice(url)
                })
            });
        }
        ...
        $scope.editInvoice = function () {
            ...
            $state.go('app.edit-invoice', {
                order: $scope.order, client: $scope.clientData,
    ...
    .controller('EditInvoiceCtrl', function ($scope, $stateParams, $rootScope, $stateParams, $ionicModal, $ionicHistory, $ionicPopup, $ionicLoading, $cordovaDialogs, $cordovaEmailComposer, InvoiceService, Server) {
        ...
        if ($scope.order.ship_info) {
            ...
            $scope.ship_info = {
                name: '', company: '', addresss: '', city: '', zipcode: '', phone: ''};
        }
    ...
    .controller('OrderAddCtrl', function ($scope, $state, $stateParams, $ionicModal, $ionicLoading, $cordovaDialogs, $ionicHistory, $rootScope, _, Server, ionicDatePicker, $timeout) {
        ...
        $scope.items = [{id: 0, title: "Sales Tax", model: 0, modeltitle: "sales_tax", show: true}, {
            id: 1,
            title: "Discount",
            model: 0,
            modeltitle: "discount",
        ...
        $scope.changedProduct = function (value, index) {
            $scope.order.products[index] = {
                product_id: value._id, distributor_id: value.distributor_id,
    ...
    .controller('DistributorInvoiceCtrl', function ($scope, $stateParams, $rootScope, $ionicModal, $ionicLoading, $ionicPopup, $cordovaToast, $cordovaEmailComposer, $cordovaDialogs, $filter, Server, InvoiceService, Server) {
    ...
    .controller('ControlPanelCtrl', function ($scope, ImageService) {
